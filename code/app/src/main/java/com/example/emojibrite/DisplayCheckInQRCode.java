...
import com.example.emojibrite.Database;
...
public class DisplayCheckInQRCode extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button generateButton = findViewById(R.id.upload_button);
        Button uploadButton = findViewById(R.id.generate_button);
        ImageView qrcodeImage = findViewById(R.id.event_qr_pic_check_in);
        FloatingActionButton backButton = findViewById(R.id.floatingActionButton_back_checkin_image);
        Button shareButton = findViewById(R.id.share_button_check_in1);
        ...
        database.getEventById(eventId, event -> {
            if (event != null && event.getCheckInQRCode() != null) {
                // Run the image loading on the main thread
                new Handler(Looper.getMainLooper()).post(() -> {
                    // Load the QR code image into the ImageView
                    Glide.with(DisplayCheckInQRCode.this)
                            .load(event.getCheckInQRCode().toString())
                            .into(qrcodeImage);
                });
            } else {
        ...
        shareButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                new QRGeneratorSharing(DisplayCheckInQRCode.this, true).shareQRImage(bitmap);
            }
        });
    ...
    private void downloadImage(ImageView imageView) {
        ...
        try {
            ...
            fos.flush();
            fos.close();
            Toast.makeText(DisplayCheckInQRCode.this, "Image Downloaded", Toast.LENGTH_LONG).show();
        } catch (Exception e) {
            Toast.makeText(DisplayCheckInQRCode.this, "Error: " + e.getMessage(), Toast.LENGTH_LONG).show();
        }
    }
}
